...
### Canonical comparison + payout row sets (locked order & labels)
Every firm page uses these exact rows, in this order, with the left column static and the right column the firm's value. Uniform phrasing across firms is the whole point — match wording exactly when two firms are equal (e.g. always "5 Winning Days", "$0", "Up to 5").

**Accounts › Comparison View › Evaluation toggle** (size+price buttons): Evaluation Steps · Profit Target · Total Drawdown · Daily Loss Limit · Days to Pass · Consistency Rule · Max Contracts (Yes/No) · Max Accounts (Up to N) · Account Resets (Yes/No + amount) · Activation Fee ($0 or amount) · News Trading (Yes/No + time constraint) · Bots / Algo Trading (No/Yes/Semi-automated).

**Accounts › Comparison View › Funded toggle** (size+price buttons, same as Evaluation): Max Payout · Profit Target · Total Drawdown · Daily Loss Limit · Days to Payout · Consistency Rule · Contract Scaling Plan (Yes/No) · Max Accounts · Account Resets · Profit Split · News Trading · Bots / Algo Trading.

**Payouts tab** (size buttons show each size's Max Payout): Max Payout · Max Payout (Subsequent) · Minimum Payout · Profit Target (amount or No Target) · Withdrawal Buffer (amount or No Buffer) · Total Drawdown · Daily Loss Limit · Days to Payout · Profit / Trading Day · Consistency Rule · Profit Split · Per Request (Up to X% / cap) · Payout Frequency · Processing Time.

Mechanics: comparison rows are data-driven — values via `data-field`, the 4 phase-specific labels (row1/row5/row7/row10) via `data-label-key` + `PHASE_LABELS`. Payout rows use `data-payout-field`; Select swaps Flex/Daily values (not labels). Adding a firm = filling `ACCOUNT_DATA` and `PAYOUT_DATA`, never re-typing rows.

Field conventions (keep identical across firms): Max Contracts = "Yes" when full contracts are available during eval (no in-eval scaling), else "No". Contract Scaling Plan = "Yes" only if the funded account scales contracts (Tradeify: Select Yes, Growth/Lightning No). News Trading / Bots = "Yes/No" (use "Semi-automated" only when the firm requires the trader present). Total Drawdown shows the clean dollar amount; the drawdown *type* (EOD/trailing/static) lives on the Rules tab. DLL keeps its "(EOD)"/"(Intraday)" qualifier.
